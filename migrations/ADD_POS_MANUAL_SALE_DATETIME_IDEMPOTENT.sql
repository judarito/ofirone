-- =============================================================================
-- ADD_POS_MANUAL_SALE_DATETIME_IDEMPOTENT.sql
-- Objetivo:
--   Extender sp_create_sale_idempotent para soportar p_sold_at sin perder la
--   mejora de mensajes legibles en errores de stock/variantes.
--
-- Requiere:
--   - Ejecutar primero la migracion backend ADD_POS_MANUAL_SALE_DATETIME.sql
--     que extiende public.sp_create_sale con el parametro p_sold_at.
-- =============================================================================

DROP FUNCTION IF EXISTS sp_create_sale_idempotent(TEXT, UUID, UUID, UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID);
    ...
    WHEN OTHERS THEN
      ...
      IF v_variant_id_text IS NOT NULL THEN
        ...
        IF v_variant_id IS NOT NULL THEN
          ...
          IF FOUND THEN
            IF COALESCE(TRIM(v_product_name), '') <> ''
               AND COALESCE(TRIM(v_variant_name), '') <> ''
               AND LOWER(TRIM(v_product_name)) <> LOWER(TRIM(v_variant_name)) THEN
...
REVOKE ALL ON FUNCTION sp_create_sale_idempotent(TEXT, UUID, UUID, UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID, TIMESTAMPTZ) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION sp_create_sale_idempotent(TEXT, UUID, UUID, UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID, TIMESTAMPTZ) TO authenticated;
GRANT EXECUTE ON FUNCTION sp_create_sale_idempotent(TEXT, UUID, UUID, UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID, TIMESTAMPTZ) TO service_role;

COMMENT ON FUNCTION sp_create_sale_idempotent IS
'Wrapper idempotente mobile con soporte p_sold_at y mensajes legibles para errores de stock/variantes.';
