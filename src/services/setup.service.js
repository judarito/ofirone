...
export async function saveTenantConfig(tenantId, payload) {
  try {
    const settingsBody = {
      ...
      accounting_enabled: Boolean(payload.settings?.accounting_enabled),
      accounting_mode: payload.settings?.accounting_mode || 'ASYNC',
      accounting_ai_enabled: payload.settings?.accounting_ai_enabled !== false,
      accounting_auto_post_sales: Boolean(payload.settings?.accounting_auto_post_sales),
      accounting_auto_post_purchases: Boolean(payload.settings?.accounting_auto_post_purchases),
      accounting_country_code: payload.settings?.accounting_country_code || 'CO',
      ...
      pos_allow_manual_sale_datetime: payload.settings?.pos_allow_manual_sale_datetime === true,
      pos_max_backdate_hours: Number(payload.settings?.pos_max_backdate_hours || 24),
