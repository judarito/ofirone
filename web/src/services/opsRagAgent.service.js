...
export async function askOpsRagAgent({
  ...
  const { data, error } = await supabaseService.client.functions.invoke(OPS_RAG_EDGE_FUNCTION, {
    body,
  })
  ...
  if (!data?.success || !data?.data) {
    return {
      success: false,
      error: data?.error || data?.details || 'La Edge Function no devolvio un payload valido.',
      data: null,
    }
  }
