/* ============================================================================
   UPDATE_SP_CREATE_SALE_FE.sql
   sp_create_sale — VERSIÓN 6.0: Soporte Facturación Electrónica DIAN

   CAMBIOS RESPECTO A v5.0 (MANUFACTURING_SP_CREATE_SALE_INTEGRATED.sql):
   ✓ Nuevo parámetro p_third_party UUID DEFAULT NULL
     → Receptor fiscal de third_parties para FE (requerido cuando FE habilitada)
   ✓ third_party_id guardado en header de venta (sales.third_party_id)
   ✓ unit_id, standard_code, standard_code_type copiados desde product_variants
     hacia cada sale_line automáticamente al crear la venta
     → Permite al proveedor tecnológico incluir unidad DIAN y código UNSPSC en XML

   PRESERVA TODA la lógica de v5.0:
   ✓ RESELL / SERVICE / MANUFACTURED ON_DEMAND / TO_STOCK / BUNDLE
   ...
   PRERREQUISITOS:
   1. ADD_ELECTRONIC_INVOICING.sql  (columnas FE en sales, sale_lines, product_variants)
   2. ADD_FE_COMPLEMENTARY.sql      (dirección emisor, city_code, dian_payment_code)

   IDEMPOTENTE: CREATE OR REPLACE es seguro de re-ejecutar.
   ============================================================================ */

-- ─────────────────────────────────────────────────────────────────────────────
-- DROP todas las versiones sobrecargadas existentes de sp_create_sale
-- (necesario porque CREATE OR REPLACE falla cuando hay múltiples firmas)
-- Usamos IF EXISTS para que sea idempotente.
-- ─────────────────────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS sp_create_sale(UUID, UUID, UUID, UUID, UUID, JSONB, JSONB, TEXT);
DROP FUNCTION IF EXISTS sp_create_sale(UUID, UUID, UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID);
DROP FUNCTION IF EXISTS sp_create_sale(UUID, UUID, UUID, UUID, UUID, JSONB, JSONB);
...
CREATE OR REPLACE FUNCTION sp_create_sale(
  ...
  p_third_party   UUID    DEFAULT NULL   -- v6.0: receptor fiscal para FE
) RETURNS UUID
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  -- v6.0: Campos FE por línea (copiados desde product_variants)
  v_variant_unit_id  UUID;
  v_variant_std_code TEXT;
  v_variant_std_type TEXT;
...
BEGIN
  ...
  -- ─────────────────────────────────────────────────────────────────────────
  -- CREAR HEADER DE VENTA  (v6.0: agrega third_party_id)
  -- ─────────────────────────────────────────────────────────────────────────
  INSERT INTO sales(
    ...
    third_party_id   -- v6.0: receptor fiscal para FE
  )
  VALUES (
  ...
  LOOP
    ...
    -- ═════════════════════════════════════════════════════════════
    -- DETECTAR COMPORTAMIENTO DE MANUFACTURA
    -- ═════════════════════════════════════════════════════════════
    ...
    -- ─────────────────────────────────────────────────────────────
    -- v6.0: Obtener costo + FE fields en un solo SELECT
    -- ─────────────────────────────────────────────────────────────
    SELECT pv.cost,
           COALESCE(pv.allow_backorder, FALSE),
           pv.unit_id,
           pv.standard_code,
           COALESCE(pv.standard_code_type, 'UNSPSC')
    INTO   v_cost, v_allow_backorder,
           v_variant_unit_id, v_variant_std_code, v_variant_std_type
    ...
    -- v6.0: incluye unit_id, standard_code, standard_code_type
    -- ═════════════════════════════════════════════════════════════
    INSERT INTO sale_lines(
      tenant_id, sale_id, variant_id, quantity,
      unit_price, unit_cost, discount_amount,
      tax_amount, line_total, tax_detail,
      unit_id, standard_code, standard_code_type   -- v6.0: datos DIAN para FE
    )
    VALUES (
      ...
      v_variant_unit_id, v_variant_std_code, v_variant_std_type   -- v6.0
    )
    ...
    ELSIF v_behavior = 'MANUFACTURED' AND v_production_type = 'ON_DEMAND' THEN
      ...
      -- Actualizar solo precio sugerido para referencia.
      DECLARE
        v_unit_cost NUMERIC;
      ...
      BEGIN
        ...
        IF v_unit_cost IS NOT NULL THEN
          v_suggested_price := fn_calculate_price(p_tenant, v_variant, v_unit_cost, p_location);

          UPDATE product_variants
          SET suggested_price = v_suggested_price,
              updated_at = NOW()
    ...
    ELSE
      -- RESELL o MANUFACTURED TO_STOCK: Consumir producto con FEFO
      IF v_allocation.allocation_details IS NOT NULL THEN
        FOR v_batch IN
          SELECT * FROM jsonb_to_recordset(v_allocation.allocation_details) AS x(
            batch_id UUID,
...
COMMENT ON FUNCTION sp_create_sale(UUID, UUID, UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID) IS
  'v6.0: Crea venta con soporte FE (third_party_id + unit_id/standard_code en líneas). '
  'Soporta RESELL, SERVICE, MANUFACTURED (ON_DEMAND/TO_STOCK), BUNDLE. '
  'Incluye FEFO, redondeo, discount_type, price_includes_tax.';
...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '════════════════════════════════════════════════════════';
  RAISE NOTICE '✅ sp_create_sale v6.0 — FE Support';
  RAISE NOTICE '════════════════════════════════════════════════════════';
  RAISE NOTICE '  ✓ p_third_party UUID DEFAULT NULL (receptor fiscal FE)';
  RAISE NOTICE '  ✓ sales.third_party_id se guarda en creación';
  RAISE NOTICE '  ✓ sale_lines.unit_id copiado desde product_variants';
  RAISE NOTICE '  ✓ sale_lines.standard_code copiado desde product_variants';
  RAISE NOTICE '  ✓ sale_lines.standard_code_type copiado desde product_variants';
  RAISE NOTICE '  ✓ Toda la lógica de manufactura y FEFO preservada';
  RAISE NOTICE '════════════════════════════════════════════════════════';
END;
$$ LANGUAGE plpgsql;
