...
-- Eliminamos firmas previas para evitar ambiguedad cuando la BD ya trae
-- varias versiones historicas de sp_create_sale.
...
BEGIN
  ...
  LOOP
    ...
    ELSIF v_behavior = 'MANUFACTURED' AND v_production_type = 'ON_DEMAND' THEN
      ...
      -- El costo real queda congelado en sale_lines.production_cost y no
      -- se pisa ni el costo estándar ni el precio catálogo automáticamente.
      DECLARE
        v_unit_cost NUMERIC;
...
COMMENT ON FUNCTION sp_create_sale(UUID, UUID, UUID, UUID, UUID, JSONB, JSONB, TEXT) IS
